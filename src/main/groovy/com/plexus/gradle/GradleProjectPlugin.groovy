...
//    Plexus Android SDK compatible with the app's project
...
class GradleProjectPlugin implements Plugin<Project> {
    ...
    static final String GROUP_PLEXUS = 'gradle.plexus'
    ...
    //           Example: If firebase-iid:20.1.6 is used in a project then Plexus will still be upgraded to 3.13.0
    //                    even though Plexus:3.13.0 is only defined in the hash under firebase-iid:20.1.1.
    ...
    static boolean didUpdatePlexusVersion
    ...
    void apply(Project inProject) {
        project = inProject
        project.logger.info('Initializing Plexus-Gradle-Plugin 0.12.10')

        hasFullPlayServices = false
        gradleV2PostAGPApplyFallback = false
        didUpdatePlexusVersion = false
    ...
    // At fundamental level this Plexus plugin and the gms version checks are solving the same problem
    ...
    static void detectProjectState() {
        ...
        if (isAGPVersionOlderThan(plugin, '3.0.0')) {
            project.logger.warn("WARNING: The plexus-gradle-plugin MUST be before com.android.application!")
            project.logger.warn("   Please put plexus-gradle-plugin first OR update to com.android.tools.build:gradle:3.0.0 or newer!")
    ...
    static boolean isAGPVersionOlderThan(Plugin plugin, String version) {
        ...
        if (!agpVersion) {
            if (project)
                project.logger.warn('Plexus Gradle Warning: Could not get AGP plugin version')
            return false
        }
    ...
    static void warnOnce(WarningType type, String msg) {
        ...
        project.logger.warn("PlexusPlugin: WARNING: $msg")
    }
        ...
        MINIMUM_MODULE_VERSION_FOR_TARGET_SDK.each { groupKey, targetSdkMap ->
            ...
            targetSdkMap.each { limitTargetSdkVersion, groupVersion ->
                ...
                if (newVersion != curVersion) {
                    project.logger.info("Changing Plexus for TargetSdkVersion. ${curVersion} -> ${newVersion}")
    ...
    static void compileSdkVersionDependencyLimits(versionOverride) {
        ...
        if (newMaxVersion != currentOverride) {
            warnOnce(
                    ...
                    "PlexusPlugin: Downgraded 'com.android.support:$currentOverride' -> $newMaxVersion" +
    ...
    static void overrideVersion(DependencyResolveDetails details, String groupVersionOverride) {
        ...
        if (details.respondsTo('because'))
            details.because('Plexus override')
    }
    ...
    static void logModuleOverride(DependencyResolveDetails details, String resolvedVersion) {
        ...
        project.logger.info("PlexusProjectPlugin: ${msg}")
    }
    ...
    static Map<String, Object> finalAlignmentRules() {
        project.logger.info("PlexusProjectPlugin: FINAL ALIGN PART 1: Groups : ${versionGroupAligns}")
        project.logger.info("PlexusProjectPlugin: FINAL ALIGN PART 1: Modules: ${versionModuleAligns}")
        ...
        project.logger.debug("PlexusProjectPlugin: FINAL ALIGN PART 2: ${finalVersionGroupAligns}")
    ...
    static void generateHighestVersionsForGroups(Configuration configuration) {
        ...
        configCopy.resolutionStrategy.eachDependency { DependencyResolveDetails details ->
            ...
            // Needed for "Upgrade to compatible Plexus SDK when using Android Support library rev 27" test
        ...
        // Plexus version was changed, need to rerun to get it's dependencies
        if (didUpdatePlexusVersion) {
            versionGroupAligns.each { group, settings ->
                if (group != GROUP_PLEXUS)
                    settings['version'] = NO_REF_VERSION
            }

            project.logger.info("didUpdatePlexusVersion changed, doing a 2nd pass")
    ...
    static void updateVersionGroupAligns(String group, String version) {
        if (group == GROUP_PLEXUS)
            project.logger.info("Plexus: Setting version in versionGroupAligns to: ${version}")
     ...
     * @param module - String - Gradle module such as "Plexus"
            ...
            value.each { parentGroupEntry ->
                ...
                if (parentGroupVersionEntry != null) {
                    ...
                    if (compareVersionResult != parentGroupVersionEntry['version']) {
                        if (parentGroupEntry.key == GROUP_PLEXUS)
                            didUpdatePlexusVersion = true
